...
from .utils import EUPE_BASE_URL
...
def _make_eupe_vit_model_url(
    *,
    ...
    model_name = "eupe"
    model_arch = _make_eupe_vit_model_arch(patch_size=patch_size, compact_arch_name=compact_arch_name)
    ...
    return os.path.join(EUPE_BASE_URL, model_dir, model_filename)
...
def _make_eupe_vit(
    ...
    **kwargs,
    ...
    if pretrained:
        ...
        print(f"missing keys when loading {compact_arch_name}: {m}")
        print(f"unexpected keys when loading {compact_arch_name}: {u}")
...
def _make_eupe_convnext_model_url(
    *,
    ...
    **kwargs,
    ...
    if pretrained:
        if type(weights) is Weights and weights not in {Weights.LVD1689M, Weights.SAT493M}:
            ...
            url = _make_eupe_convnext_model_url(
                compact_arch_name=compact_arch_name,
                weights=weights,
                hash=hash,
            )
