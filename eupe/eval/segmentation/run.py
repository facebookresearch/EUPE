...
from eupe.eval.segmentation.config import SegmentationConfig
from eupe.eval.segmentation.eval import test_segmentation
from eupe.eval.segmentation.train import train_segmentation
...
def run_segmentation_with_eupe(
    backbone,
    config,
):
    if config.load_from:
...
def benchmark_launcher(eval_args: dict[str, object]) -> dict[str, Any]:
    ...
    if dataclass_config.model:
        ...
        raise ValueError("No model config or load_from specified")
    ...
    results_dict = run_segmentation_with_eupe(backbone=backbone, config=dataclass_config)
