...
from eupe.data.transforms import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
...
class LeftRightFlipAug(Aug):
    ...
    def inverse(self, stacked_left_right_pair: torch.Tensor) -> torch.Tensor:
        ...
        return (
            stacked_left_right_pair[:pre_aug_batch_size]
            + TF.hflip(stacked_left_right_pair[pre_aug_batch_size:])
        ) / 2
...
class NormalizeDepth(torch.nn.Module):
    def __init__(self, normalization_factor):
        ...
        assert (
            self.factor > 1e-6
        ), f"Normalization factor should be > 1e-6, got {self.factor}"

    def forward(self, img, label):
        assert label is not None
        label = Depth(
            label / self.factor
        )  # have to rewrap otherwise it becomes a torch.Tensor
        return img, label
...
class ResizeV2:
    ...
    def __call__(self, img, label):
        ...
        if self.resize_label:
            label = T.Resize(size=self.size, interpolation=self.label_interpolation)(
                label
            )
        return img, label
...
def depth_resize(my_dp, size):
    out = TF.resize(
        my_dp, size=size, interpolation=T.InterpolationMode.NEAREST, antialias=True
    )
    ...
    transforms_list.append(
        MaybeApply(
            v2.Compose(
                [
                    v2.RandomRotation(
                        degrees=rotation_angle, interpolation=interpolation
                    ),
                ]
            ),
            threshold=0.5,
