...
from eupe.eval.depth.checkpoint_utils import (
    find_latest_checkpoint,
    load_checkpoint,
    save_checkpoint,
)
from eupe.eval.depth.config import (
    DepthConfig,
    make_depth_eval_transforms_from_config,
    make_depth_train_transforms_from_config,
    ResultConfig,
)
from eupe.eval.depth.data import build_dataloader
from eupe.eval.depth.datasets.datasets_utils import _EvalCropType, make_valid_mask
from eupe.eval.depth.eval import evaluate_depther_with_dataloader
from eupe.eval.depth.loss import MultiLoss
from eupe.eval.depth.metrics import DEPTH_METRICS
from eupe.eval.depth.models import Depther, make_depther_from_config
from eupe.eval.depth.schedulers import build_scheduler

from eupe.eval.depth.utils import setup_model_ddp
from eupe.logging import MetricLogger, SmoothedValue
from eupe.utils import fix_random_seeds
...
class IterBasedTrainer:
    def __init__(
        ...
        self._train_dataset_epoch = (
            0  # a counter for how many times all images in the dataset were seen
        )
    ...
    def train_on_batch(self, batch):
        ...
        # c) forward pass
        pred = self.depther(batch_img.float())
...
def run_epochs(
    config: DepthConfig, backbone: torch.nn.Module, autocast_dtype: torch.dtype
):
    ...
    if config.load_from is None:
        config.load_from = find_latest_checkpoint(
            config.output_dir
    ...
    logger.info(
        f"Built trainer with start: {trainer.global_step} | total_iter {trainer.total_iter}"
    )
...
def train_model_with_backbone(
    ...
    trained_model = run_epochs(
        config=config, backbone=backbone, autocast_dtype=autocast_dtype
    )
