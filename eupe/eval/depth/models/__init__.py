...
import torch
from eupe.eval.depth.checkpoint_utils import load_checkpoint
...
class DecoderConfig:
    ...
    max_depth: float = 80
    bins_strategy: str = (
        "linear"  # (choice: ["linear", "log"]) distribution of bins across the range
    )
    norm_strategy: str = (
    ...
    adapt_to_patch_size: PatchSizeAdaptationStrategy = (
        PatchSizeAdaptationStrategy.CENTER_PADDING
    )
...
def make_head(
    ...
    **kwargs,
    ...
    if head_type == "linear":
        ...
        raise NotImplementedError("only linear head supported")
    return decoder
