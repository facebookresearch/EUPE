...
import torch
from eupe.eval.depth.checkpoint_utils import find_latest_checkpoint
from eupe.eval.depth.config import DepthConfig
from eupe.eval.depth.eval import evaluate_depther_with_config
from eupe.eval.depth.models import make_depther_from_config
from eupe.eval.depth.train import train_model_with_backbone

from eupe.eval.helpers import args_dict_to_dataclass, cli_parser, write_results
...
def eval_depther_with_model(*, depther: torch.nn.Module, config: DepthConfig):
    ...
    for metric, values in results_dict.items():
        results_dict[metric] = float(
            torch.Tensor(values).nanmean()
        )  # result can be NaN if ground truth is all masked
    summary = " \n====== Summary ======\n"
    summary += (
...
def benchmark_launcher(eval_args: dict[str, Any]) -> dict[str, Any]:
    ...
    config_autocast_dtype = (
        depth_config.model_dtype.autocast_dtype
        if depth_config.model_dtype is not None
        else None
    )
    ...
    with torch.device("cuda" if torch.cuda.is_available() else "cpu"):
        ...
        model, model_context = load_model_and_context(
            depth_config.model, output_dir=output_dir
        )
